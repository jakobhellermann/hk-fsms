<script lang="ts">
	...
	// path-based hrefs: /[game] (scenes) and /[game]/[scene] (scene = bundle file, slashes encoded)
	function hrefFor(opts: { game?: Game; scene?: string }) {
		const g = opts.game ?? game;
		return opts.scene ? `${base}/${g}/${encodeURIComponent(opts.scene)}` : `${base}/${g}`;
	}
	const fsmHref = (hash: string) => `${base}/${game}/fsm/${hash}`;
