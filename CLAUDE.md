...
- Lives in `static/data/<game>/`: `index.json` (one entry per FSM: `file`, `path_id`, `name`,
  `game_object`, `hash`), `scenes.json` (`{file: scene_name}` map), and `content/<hash>.json`
  (the serialized model, deduped by content hash).
...
- `src/lib/data.ts` — re-exports from `config.ts` + fetchers (`fetchIndex`, `fetchSceneNames`,
  `fetchModel`) and URL path helpers (`fsmSegments`, `resolveFsm`, `goLeaf`).
